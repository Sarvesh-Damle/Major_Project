...
const HostelProperties = () => {
  ...
  const handleSelectFilter = async (filterName, value) => {
    switch (filterName) {
      ...
      case 'Sharing Type':
        ...
        break;
      case 'By Locality':
        newSortedData = newSortedData.filter(property => property.locality.toLowerCase().includes(value.toLowerCase()));
        break;
      default:
        break;
    }
  ...
  const propertiesData = filteredData || data.data.filter((property) =>
        ...
        <div className="flexColCenter p-6 innerWidth gap-8 properties-container">
          ...
          <div className="flex w-full gap-x-10 gap-y-4 flex-wrap filters">
            <div className="flex w-full gap-8 justify-center items-center">
              <CheckBoxDropdown dropdownTitle="Type of Flats" items={items_type_of_hostels} selectedItems={typeOfHostel} onItemsSelected={(value) => { setTypeOfHostel(value); handleSelectFilter('Type of Hostel', value); }} />
              <CheckBoxDropdown dropdownTitle="Sharing Type" items={items_sharingType} selectedItems={sharingType} onItemsSelected={(value) => { setSharingType(value); handleSelectFilter('Sharing Type', value); }} />

              <DropdownSelect title="Sort" options={sort} onSelect={(title, value) => { setDropdownValueSort(value); handleSelectFilter(title, value); }} dropdownValue={dropdownValueSort} setDropdownValue={setDropdownValueSort} />
              <DropdownSelect title="By Locality" options={localities} onSelect={(title, value) => { setDropdownValueLocality(value); handleSelectFilter(title, value); }} dropdownValue={dropdownValueLocality} setDropdownValue={setDropdownValueLocality} />
            </div>
          </div>
          <div className="p-6 w-full flex justify-center items-center gap-y-8 gap-9 flex-wrap properties">
            {propertiesData && propertiesData
              ...
              .map((card, index) => {
                return (
                  <PropertiesCard card={card} key={index} />
                )
              })}
...
export const DropdownSelect = ({ title, options, onSelect, dropdownValue, setDropdownValue }) => {
  ...
  return (
    <div className="relative inline-block text-left z-10">
      <button onClick={toggleDropdown} id="dropdownDefaultButton" className="bg-white border border-gray-500 hover:bg-blue-800 rounded-3xl font-medium px-6 py-3 text-center inline-flex items-center transition-all duration-200 ease-in hover:cursor-pointer transform hover:scale-95" type="button">
        {dropdownValue || title}
        {isOpen ?
          (<svg
      ...
      {isOpen && (
        <div className="absolute mt-1 w-44 bg-white divide-y divide-gray-100 rounded-lg font-medium shadow z-10">
          <ul className="p-3 space-y-1 text-sm text-gray-700" aria-labelledby="dropdownDefaultButton">
            {options && options.map((option, index) => (
              <li key={index} className="hover:cursor-pointer p-2 hover:bg-gray-100 rounded-lg pl-4" onClick={() => handleSelect(option)}>
                {option}
              </li>
            )
            )}
...
export const CheckBoxDropdown = ({ dropdownTitle, items, selectedItems, onItemsSelected }) => {
  ...
  return (
    <div className="relative inline-block text-left">
      <button onClick={() => setIsOpen(!isOpen)} id="dropdownBgHoverButton" className="bg-white border border-gray-500 hover:bg-blue-800 rounded-3xl font-medium px-6 py-3 text-center inline-flex items-center transition-all duration-200 ease-in hover:cursor-pointer transform hover:scale-95" type="button">
        {dropdownTitle}
        {isOpen ?
          (<svg
            className="w-2.5 h-2.5 ms-3"
            ...
            {items.map((item) => (
              <li key={item}>
                <div className="flex items-center p-2 rounded hover:bg-gray-100">
                  ...
                  />
                  <label htmlFor={`checkbox-item-${item}`} className="ml-2 text-sm font-medium text-gray-900">
                    {item}
                  </label>
                </div>
              </li>
