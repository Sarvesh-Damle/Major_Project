...
const HostelProperty = () => {
  ...
  const { liked, toggleFavorite } = useFavorite(id, 'hostel');
  ...
  useTrackPropertyView(property, 'hostel');
  ...
  return (
    <div className='flex flex-wrap justify-center w-full h-full my-8 singlePage'>
          ...
          <div className='mt-12 info'>
            <div className='flex justify-between top'>
              <div className='flex flex-col gap-5 post'>
                <h1 className='font-semibold text-2xl'>{property.hostel_name}</h1>
            ...
            <div className='flex items-center gap-4 mt-10'>
              ...
              <ShareButtons
                title={property.hostel_name}
                text={`Check out ${property.hostel_name} - Rs. ${property.rent_amount}/month at ${property.address}`}
              />
            </div>
          </div>
        </div>
        ...
        <div className='py-5 flex flex-col gap-5 wrapper'>
          ...
          <div className='flex gap-5 py-5 px-2 mx-3 bg-white rounded-[10px] rooms'>
            <div className='flex items-center gap-[10px] room'>
              <FaBuilding />
              <p>{property.type_of_hostel}</p>
            </div>
            <div className='flex items-center gap-[10px] room'>
              <IoBed />
              <div className='flex gap-5'>
                {property.room_type.map((type, index) => (
                  <p key={index}>{type}</p>
                ))}
              </div>
            </div>
      ...
      <div className='w-full px-8 pb-8'>
        <ReviewList propertyId={id} propertyType='hostel' />
      </div>
    </div>
  );
};
