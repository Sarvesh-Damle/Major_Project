...
const Contact = () => {
  ...
  const mutation = useMutation({
    mutationKey: ['contact'],
    mutationFn: (formData) => {
      return axios.post("/api/v1/contact", formData, { withCredentials: true }) 
    },
    onSuccess(data) {
      navigate("/")
          ...
          <div className="w-full flex justify-center px-4 lg:w-1/2 xl:w-6/12">
            <div className="mb-12 max-w-[570px] lg:mb-0">
              ...
              <p className="text-base leading-relaxed mb-9 text-body-color">
                For rental inquiries or to learn more about our properties, please reach out to us using the contact information below. We look forward to assisting you in finding your ideal rental space.
              </p>
              <div className="mb-8 flex w-full max-w-[370px]">
                <div className="mr-6 flex h-[60px] w-full max-w-[60px] items-center justify-center overflow-hidden rounded bg-primary bg-opacity-5 text-white sm:h-[70px] sm:max-w-[70px]">
                  <svg
                    width={24}
                    height={24}
                    viewBox="0 0 24 24"
              ...
              <form onSubmit={handleSubmit(formData => mutation.mutate(formData))}>
                ...
                <div className="mb-6">
                  <Controller
                    ...
                    render={({ field }) => (
                      <>
                        <textarea {...field} title="message" placeholder="Your Message..." className="border-[f0f0f0] w-full resize-none rounded border py-3 px-[14px] text-base text-body-color outline-none focus:border-primary focus-visible:shadow-none " rows={4} />
                        {errors && errors.message && <p className='text-red-600'>{errors.message.message}</p>}
                      </>
                    )}
                  />
                </div>
                <div>
                  <button
                    ...
                    className={`w-full p-3 text-white transition border rounded border-primary bg-primary ${mutation.isPending ? 'opacity-50 cursor-not-allowed' : 'hover:bg-opacity-90'}`}
                  >
