...
const Profile = () => {
    ...
    const { data, isLoading, isError, refetch: refetchUserDetails } = useQuery({
    ...
    const { data: favourites, isLoading: isLoadingFavourites, isError: isErrorFavourites, refetch: refetchFavourites } = useQuery({
        queryKey: ["Favourites"],
        queryFn: async () => {
        ...
        <>
            <div className="flex h-full flex-wrap">
                <div className="w-60 max-md:w-full bg-gray-100 p-4 text-xl flex flex-col gap-5">
                    <SidebarItem icon={<HiUser />} label="User Details" onClick={() => handleItemClick("User Details")} />
                    <SidebarItem icon={<HiShoppingBag />} label="See Favourites" onClick={() => handleItemClick("See Favourites")} />
                    {data.data.isAdmin && <SidebarItem icon={<HiChartPie />} label="View Dashboard" onClick={() => handleRedirect()} />}
                    <SidebarItem icon={<FaIndianRupeeSign />} label="My Credits" onClick={() => handleItemClick("My Credits")} />
                </div>
                <div className="flex-1 p-4">
                    {selectedItem === "User Details" && (<UserProfile data={data} refetchUserDetails={refetchUserDetails} />)}
                    {selectedItem === "See Favourites" && <Favourites favourites={favourites} removeFavouritesMutation={removeFavouritesMutation} selectedItem={selectedItem} />}
...
const UserProfile = ({ data, refetchUserDetails }) => {
    ...
    const mutation = useMutation({
        ...
        mutationFn: () => {
            return axios.put(`/api/v1/users/update-user?id=${id}`, { name, email, phoneNumber },
...
const SidebarItem = ({ icon, label, onClick }) => {
    return (
        <div className="flex items-center cursor-pointer p-2 rounded-md hover:bg-gray-200" onClick={onClick}>
...
const Favourites = ({ favourites, removeFavouritesMutation, selectedItem }) => {
    return (
        <div className="text-black">{((favourites?.data.hostel === undefined || favourites?.data.hostel.length === 0) && (favourites?.data.pg === undefined || favourites?.data.pg.length === 0) && (favourites?.data.flat === undefined || favourites?.data.flat.length === 0)) ? <div className="flex justify-start p-2 mt-10 items-center">Empty Favourites</div> : <>{selectedItem === "See Favourites" && <div className="p-6 w-full flex flex-col justify-center items-center gap-2 overflow-hidden r-container">
