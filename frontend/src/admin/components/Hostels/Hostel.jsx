...
const Hostel = () => {
  ...
  const handleEdit = (prop) => {
    console.log(prop)
    navigate.push(`/dashboard/hostels/edit-hostel/${prop._id}`);
  };
  let column = [
    ...
    { field: "featured", headerName: "Verified", filter: true, width: 100 },
    {
      ...
      width: 100,
      cellRenderer: (prop) => (
        <HostelEditButton func={handleEdit} {...prop}/>
      ),
    },
    {
      field: "Delete",
      width: 100,
      cellRenderer: (prop) => (
        <HostelDeleteButton func={handleDelete} {...prop} />
      ),
    },
  ];
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["Hostels"],
    queryFn: async () => {
      const response = await axios.get("/api/v1/hostels/find-all-hostels-info", { withCredentials: true });
  ...
  const { data: data2, isLoading: isLoading2, isError: isError2 } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get("/api/v1/hostels/count-hostel-type", { withCredentials: true });
      return response.data;
    }
  })

  if (isLoading || isLoading2) return <div className="flex h-screen w-screen justify-center items-center"><Loader /></div>
