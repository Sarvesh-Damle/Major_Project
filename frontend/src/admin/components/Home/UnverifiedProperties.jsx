...
const UnverifiedProperties = () => {
    const { data, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            const response = await axios.get("/api/v1/hostels/count-unverified-hostels", { withCredentials: true });
    ...
    const { data: data2, isLoading: isLoading2, isError: isError2 } = useQuery({
        ...
        queryFn: async () => {
            const response = await axios.get("/api/v1/pgs/count-unverified-pgs", { withCredentials: true });
            return response.data;
        }
    })
    const { data: data3, isLoading: isLoading3, isError: isError3 } = useQuery({
        ...
        queryFn: async () => {
            const response = await axios.get("/api/v1/flats/count-unverified-flats", { withCredentials: true });
            return response.data;
        }
    })
