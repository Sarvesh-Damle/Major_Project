...
const EditUser = () => {
    ...
    const { data, isLoading, isError, refetch } = useQuery({
        queryKey: ["User"],
        queryFn: async () => {
            const response = await axios.get(`/api/v1/users/get-user-info?id=${id}`, { withCredentials: true });
    ...
    const mutation = useMutation({
        ...
        mutationFn: () => {
          return axios.put(`/api/v1/users/update-user?id=${id}`,{name, email, admin, phoneNumber},
            ...
            <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
                <dl className="sm:divide-y sm:divide-gray-200">
                    ...
                    <div className="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd className="mt-1 text-sm font-medium text-gray-700 sm:mt-0 sm:col-span-2">
                            (+91) <input type="number" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} />
                        </dd>
                    </div>
                    <div className="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt className="text-sm font-medium text-gray-500">
                        ...
                        <dd className="mt-1 text-sm font-medium flex gap-x-6 text-gray-700 sm:mt-0 sm:col-span-2">
                            <div className="flex gap-x-2">
                                ...
                                <input type="radio" value="Yes" id="isAdmin" name="admin" checked={admin === true} onClick={e => e.target.value === "Yes" ? setAdmin(true) : null} />
                            </div>
                            <div className="flex gap-x-2">
                                <label htmlFor="notAdmin">No</label>
                                <input type="radio" value="No" id="notAdmin" name="admin" checked={admin === false} onClick={e => e.target.value === "No" ? setAdmin(false) : null} />
                            </div>
                        </dd>
                    </div>
                </dl>
