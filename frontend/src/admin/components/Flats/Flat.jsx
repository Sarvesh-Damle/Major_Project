...
const Flat = () => {
  ...
  const mutation = useMutation({
    ...
    mutationFn: (prop) => {
      return axios.delete(`/api/v1/flats/delete-flat?id=${prop._id}`,
  ...
  const handleEdit = (prop) => {
    console.log(prop)
    navigate.push(`/dashboard/flats/edit-flat/${prop._id}`);
  };
  let column = [
    ...
    { field: "featured", headerName: "Verified", filter: true, width: 100 },
    {
      ...
      width: 100,
      cellRenderer: (prop) => (
        <FlatEditButton func={handleEdit} {...prop}/>
      ),
    },
    {
      field: "Delete",
      width: 100,
      cellRenderer: (prop) => (
        <FlatDeleteButton func={handleDelete} {...prop} />
      ),
    },
  ];
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["Flats"],
    queryFn: async () => {
      const response = await axios.get("/api/v1/flats/find-all-flats-info", { withCredentials: true });
