...
const EditFlat = () => {
    ...
    const { data, isLoading, isError, refetch } = useQuery({
        queryKey: ["Flat"],
        queryFn: async () => {
            const response = await axios.get(`/api/v1/flats/find-flat?id=${id}`, { withCredentials: true });
    ...
    const mutation = useMutation({
        ...
        mutationFn: () => {
          return axios.put(`/api/v1/flats/update-flat?id=${id}`,{flatType, preferredTennats, flatArea, furnishedStatus, locality, rentAmount, securityDeposit, verified, city, ownerName, ownerEmail, ownerPhoneNumber, pincode, address, distanceFromRailwayStation, distanceFromBusStop, description},
            {withCredentials: true}
          )
        },
        onSuccess (data) {
          toast.success(data.data.message);
          navigate.push("/dashboard/flats");
        },
        onError (error) {
            ...
            <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
                <dl className="sm:divide-y sm:divide-gray-200">
                    ...
                    <div className="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd className="mt-1 text-sm font-medium text-gray-700 sm:mt-0 sm:col-span-2">
                            <input type="text" value={furnishedStatus} onChange={e => setFurnishedStatus(e.target.value)} />
                        </dd>
                    </div>
                    <div className="py-3 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt className="text-sm font-medium text-gray-500">
